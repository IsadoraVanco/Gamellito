...
QFile *Arquivos::abrirArquivoLeitura(QString pathArquivo){
    QFile *arquivo = new QFile(QDir::current().absoluteFilePath(pathArquivo));

    if(arquivo->open(QIODevice::ReadOnly | QIODevice::Text)){
        return arquivo;
    }else{
        qDebug() << "[Arquivos][ERRO] Falha ao abrir arquivo para leitura:" << pathArquivo;
        return nullptr;
    }
}
...
QString Arquivos::carregarPropriedadeJson(QString pathArquivo, QString propriedade){
    ...
    if (!jsonObject.contains(propriedade)) {
        qDebug() << "[Arquivos][INFO] A propriedade " + propriedade + " não foi encontrada no arquivo " + pathArquivo;
        return "";
    }
...
QJsonObject Arquivos::carregarObjetoJson(QString pathArquivo){
    // Carrega o arquivo
    QFile *arquivo = abrirArquivoLeitura(pathArquivo);
...
bool Arquivos::escreverObjetoJson(QJsonObject &objetoJson, QString pathArquivo){
    ...
    QFile *arquivoJson = abrirArquivoEscrita(pathArquivo);

    if(arquivoJson != nullptr){
        arquivoJson->write(docJson.toJson());
        fecharArquivo(arquivoJson);

        qDebug() << "[Arquivos][OK] Objeto JSON salvo com sucesso:" << pathArquivo;
        return true;
    }else{
        qDebug() << "[Arquivos][ERRO] Erro ao escrever o objeto JSON:" << pathArquivo;
        return false;
    }
}
...
QJsonArray Arquivos::converterJsonParaArray(QString pathArquivoJson){
    QFile *arquivo = abrirArquivoLeitura(pathArquivoJson);
...
void Arquivos::escreverArrayJson(QString pathArquivoJson, QJsonArray arrayJson){
    ...
    QFile *arquivo = abrirArquivoEscrita(pathArquivoJson);
    ...
    qDebug() << "[Arquivos][OK] Array JSON escrito em:" << pathArquivoJson;
}
...
void Arquivos::aumentarVisitas(){
    ...
    if(escreverObjetoJson(objetoJson, ARQUIVO_CONFIGURACAO_GERAL)) {
        qDebug() << "[Arquivos][OK] Número de visitas atualizado:" << visitas;
    } else {
...
void Arquivos::aumentarReproducoes(){
    ...
    // Salva o Json modificado de volta no arquivo
    if(escreverObjetoJson(objetoJson, ARQUIVO_CONFIGURACAO_GERAL)) {
        qDebug() << "[Arquivos][OK] Número de reproduções atualizado:" << reproducoes;
    } else {
...
void Arquivos::alterarSenha(QString novaSenha){
    ...
    // Salva o Json modificado de volta no arquivo
    if(escreverObjetoJson(objetoJson, ARQUIVO_CONFIGURACAO_GERAL)) {
        qDebug() << "[Arquivos][OK] Senha atualizada no arquivo";
    } else {
        qDebug() << "[Arquivos][ERRO] Falha ao atualizar a senha no arquivo";
    }
}
...
void Arquivos::salvarSequenciaNoArquivo(QJsonArray sequencia, QString nomeArquivo){
    ...
    qDebug() << "[Arquivos][OK] Sequência salva no arquivo:" << pathArquivo;
}
...
//        qDebug() << "valor " + QString::number(valor);
    ...
    qDebug() << "[Arquivos][OK] A resposta foi salva no arquivo:" << nomeArquivo;
}
